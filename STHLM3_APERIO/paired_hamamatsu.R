...
if (write_output){
  write.table(tab01, file = file.path('Output', 'Tables', 'PredictionSets', 'STHLM3_APERIO', 'hamamatsu_CX.csv'))
  write.table(tab05, file = file.path('Output', 'Tables', 'PredictionSets', 'STHLM3_APERIO', 'hamamatsu_CX.csv'), append = TRUE)
  write.table(tab1, file = file.path('Output', 'Tables', 'PredictionSets', 'STHLM3_APERIO', 'hamamatsu_CX.csv'), append = TRUE)
}
tab01;tab05;tab1
...
if (write_output){
  write.table(tab05, file = file.path('Output', 'Tables', 'PredictionSets', 'STHLM3_APERIO', 'hamamatsu_ISUP.csv'))
  write.table(tab10, file = file.path('Output', 'Tables', 'PredictionSets', 'STHLM3_APERIO', 'hamamatsu_ISUP.csv'), append = TRUE)
  write.table(tab20, file = file.path('Output', 'Tables', 'PredictionSets', 'STHLM3_APERIO', 'hamamatsu_ISUP.csv'), append = TRUE)
  write.table(tab33, file = file.path('Output', 'Tables', 'PredictionSets', 'STHLM3_APERIO', 'hamamatsu_ISUP.csv'), append = TRUE)
  # Cancer-detection plot
  # cx <- cx + ggtitle("New scanner")
  # ggsave(filename = file.path('Output', 'Plots', 'calibration', 'cx_hamamatsu.png'), cx, 
  #        width = 2, 
  #        height = 2)
  cx <- cx + ggtitle("")
  # Increase font-size
  cx <- cx + theme(text = element_text(size = 17))
  ggsave(filename = file.path('Output', 'Plots', 'calibration', 'nolabels', 'cx_hamamatsu.png'), cx, 
         width = 6, 
         height = 6)
}
